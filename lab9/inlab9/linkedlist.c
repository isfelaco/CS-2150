	...
	do {
		...
		switch(option){
			case PUSH_FRONT:				// push onto front of list
				my_node = (struct list_node*)malloc(sizeof(struct list_node));
				my_node->value = val;
				my_node->previous = head;
				my_node->next = head->next;
				head->next->previous = my_node;
				head->next = my_node;
				size++;
				break;
			case PUSH_BACK: 				// push onto back of list
				...
				my_node->value = val;
				my_node->next = tail;
				my_node->previous = tail->previous;
				tail->previous->next = my_node;
				tail->previous = my_node;
				size++;
				break;
			case POP_FRONT: 				// remove from front of list
				if (size != 0) {
					struct list_node* temp = head->next->next;
			...
			case POP_BACK:					// remove from back of list
				if (size != 0) {
					struct list_node* temp = tail->previous->previous;
